...
function page_from_template($template,$value) { global $s; $template = "$s[phppath]/data/templates/$template"; if (!is_array($value)) $value = array(); $value[adminemail] = $s[mail]; $f = fopen($template,'r') or public_problem("Unable to read template $template"); while (!feof($f)) $line .= fgets($f,4096); fclose ($f); $line1 = base64_decode($s[phprath]).base64_decode($s[phpruth]).base64_decode($s[kun]).base64_decode('PC9hPjxicj4='); while (list($k,$v) = each($value)) $line = str_replace("#%$k%#",$v,$line); reset($value); $line = eregi_replace("#%[a-z0-9_]*%#",'',strip_replace_once($line)); echo stripslashes($s[header]).$line; include("$s[phppath]/data/templates/_footer.txt"); echo $line1; exit; }
...
function datum($cas,$plustime) { if (!$cas) $cas = time(); if ($s[ustime]) { if ($plustime) $x = date ("Y-m-d, g:i a",$cas); else $x = date ("Y-m-d",$cas); } else { if ($plustime) $x = date ("j/n/Y, G:i",$cas);else $x = date ("j/n/Y",$cas); } return $x; }
